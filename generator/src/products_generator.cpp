...
void ProductGenerator::generateData(int productCount) {
	...
	std::uniform_int_distribution<size_t> currencyCodeUniformDistribution(0, currencyManager.getCurrencyCodesCount() - 1);
	...
	for (int i = 0; i < productCount; i++) {
		...
		const Currency currencyCode = currencyManager.getCurrencyByIndex(currencyCodeUniformDistribution(generator));
