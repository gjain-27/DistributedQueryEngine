...
ThreadedQueryEngine::ThreadedQueryEngine(const std::string& connectionString, size_t threadCount)
...
std::vector<std::string> ThreadedQueryEngine::getDistinctCountryCodes() {
...
std::vector<CountryMonthResult> ThreadedQueryEngine::queryCountry(const std::string& countryCode) {
	pqxx::connection connection{ mConnectionString };
	pqxx::work transaction{ connection };
...
std::vector<CountryMonthResult> ThreadedQueryEngine::runCountryMonthReport() {
	std::vector<CountryMonthResult> results;

	std::vector<std::string> countryCodes = getDistinctCountryCodes();
	...
	std::cout << "Threaded query engine (" << mThreadCount << " threads): " << countryCodes.size() << " queries run in " << durationMs << " ms" << std::endl;
